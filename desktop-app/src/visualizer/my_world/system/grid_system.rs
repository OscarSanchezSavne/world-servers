...
pub fn build(
    ...
    mut world_state: ResMut<NextState<my_world::data::world_data::WorldState>>,
    current_world_state: Res<State<my_world::data::world_data::WorldState>>,
    mut query: Query<
        &mut my_world::component::grid::Grid, 
        Or<(Added<my_world::component::grid::Grid>, Changed<my_world::component::grid::Grid>)>
    >,
) 
{
    if let Ok(mut grid) = query.single_mut() {
        for row in grid.matrix_cells.iter_mut() {
            for cell in row.iter_mut() {
                ...
                cell.entity= Some(my_world::entity::cell_entity::create(
                    &mut commands, &asset_server,
                    cell.clone()
                ));
            }
        }
        if *current_world_state.get() == my_world::data::world_data::WorldState::Start{
            world_state.set(my_world::data::world_data::WorldState::GridLoaded);
        }
    }
}
...
pub fn add_color_by_server_type(
    ...
    query_servers: Query<
        (
            Entity, &my_world::component::cell::Cell, &my_world::component::cell::CellType
        ),
        Added<my_world::component::cell::CellType>,
    >,
    name_query: Query<&Name>
) 
{
    for (cell_entity, cell, server_type) in query_servers {
        let color = match server_type {
            my_world::component::cell::CellType::Internal => Color::srgb(0.0, 0.2, 0.0),
            my_world::component::cell::CellType::External => Color::srgb(0.2, 0.0, 0.0),
        };
...
pub fn pulse_cells(
    ...
    mut query: Query<(
        Entity,
        &my_world::component::cell::Cell,
        &mut my_world::component::cell::PulsingCell,
        &mut Transform,
    )>,
) {
    ...
    for (entity, cell, mut pulse, mut transform) in &mut query {
        ...
        // 0 → 1 → 0
        //
        // Empieza completamente quieto,
        // se contrae suavemente y vuelve.
        ...
        // Si se pidió detener, esperamos a terminar
        // suavemente el ciclo actual.
        if pulse.stop {
            ...
            if normalized_phase < 0.02 {
                ...
                commands
                    .entity(entity)
                    .remove::<my_world::component::cell::PulsingCell>();
            }
        }
    }
}